package kalyan.dream.en.india.ui.main.view

import android.os.Bundle
import androidx.activity.viewModels
import androidx.appcompat.app.AppCompatActivity
import androidx.lifecycle.LiveData
import kalyan.dream.en.india.web_service.PaPaDetails
import kalyan.dream.en.india.web_service.model.DepositModel
import kalyan.dream.en.india.databinding.ActivityPABinding
...
import kalyan.dream.en.india.basic_utils.BasicUtils.hideKeyboard
...
class PActivity : AppCompatActivity(), PaymentStatusListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        observePaymentDetailsResponse()

    }

    private fun observePaymentDetailsResponse() {
        getPaymentDetails()?.observe(this) {
            binding.minimumDepText.text = "Minimum Deposit is Rs ${it.min_amount}"
        }
    }
    ...
    private fun initOnClick() {
        binding.apply {
            add500.setOnClickListener {
                pointsToAdd.setText("500")
            }
            add1000.setOnClickListener {
                pointsToAdd.setText("1000")
            }
            add2000.setOnClickListener {
                pointsToAdd.setText("2000")
            }
            add3000.setOnClickListener {
                pointsToAdd.setText("3000")
            }
            add5000.setOnClickListener {
                pointsToAdd.setText("5000")
            }
            add10000.setOnClickListener {
                pointsToAdd.setText("10000")
            }
            submitPoints.setOnClickListener {
                validate()
            }
        }
    }
            ...
            try {
                val easyUpiPayment = EasyUpiPayment(this) {
                    ...
                    this.transactionRefId =
                        "TREF" + (System.currentTimeMillis() / 1000).toString()
    ...
    override fun onTransactionCompleted(transactionDetails: TransactionDetails) {
        ...
        if (transactionDetails.transactionStatus == TransactionStatus.SUCCESS) {
            if (transactionDetails.responseCode == "0" || transactionDetails.responseCode == "00") {
                viewmodel.addFunds(
                    DepositModel(
